...
### A Note on the Reproducibility of Figure 5

The specific quantitative results for the B7 policy shown in **Figure 5(a)** of our submitted paper are highly sensitive to system-level environmental factors.

To provide a clear and fully reliable validation of our core argument, this artifact's script (`./run.sh fig5`) executes a **controlled experiment**. During the adversarial attack, the script deliberately throttles the background tenants' workloads to precisely simulate the "noisy neighbor" effect.

This setup produces a robust and reproducible experiment that, while quantitatively different from the paper's specific numbers, demonstrates the **same fundamental flaw** of the B7 policy. It powerfully validates our paper's qualitative conclusion about the instability of simple greedy heuristics.
